...
export default function TamTru() {
  ...
  const fetchRecords = async () => {
    try {
      const response = await api.get('/tamtru');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/tamtru', formData);
      fetchRecords();
      setShowModal(false);
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (maTamTru) => {
    if (!confirm('Bạn có chắc chắn muốn xóa bản ghi này?')) return;
    try {
      await api.delete(`/tamtru/${maTamTru}`);
      fetchRecords();
    } catch (error) {
      alert(error.response?.data?.error || 'Có lỗi xảy ra');
    }
  ...
  const filteredRecords = records.filter(
    (r) =>
      ...
      r.MaTamTru.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Quản Lý Tạm Trú</h1>
          <p className="text-gray-600 mt-2">Quản lý thông tin tạm trú của nhân khẩu</p>
        </div>
        <button
          onClick={() => {
            resetForm();
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="mb-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Tìm kiếm theo mã tạm trú hoặc CCCD..."
              value={searchTerm}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                <th className="text-left p-3">Mã Tạm Trú</th>
            ...
            <tbody>
              {filteredRecords.map((record) => (
                <tr key={record.MaTamTru} className="border-b hover:bg-gray-50">
                  <td className="p-3 font-medium">{record.MaTamTru}</td>
                  <td className="p-3">{record.SoCMND_CCCD}</td>
                  <td className="p-3">{record.LyDo}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <button
                      onClick={() => handleDelete(record.MaTamTru)}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl">
            <h2 className="text-2xl font-bold mb-4">Thêm Tạm Trú</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Mã Tạm Trú *</label>
                  <input
                    type="text"
                    value={formData.maTamTru}
                    onChange={(e) => setFormData({ ...formData, maTamTru: e.target.value })}
